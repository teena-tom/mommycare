...
function DaycarePage({ mode = "view" }) {
  ...
  const deletingDaycare = daycares?.find(
    (daycare) => daycare.id === deletingId
  );
  ...
  const handleDeleteConfirmed = (id) => {
    axios
      ...
      .then((response) => {
        setDeletingId(null);
        setDaycares(null); //Triggers a daycare  to reload
      })
      .catch((error) => {
        alert(error);
        console.error(error);
          ...
          <>
            <Link to="/">
              <button className="Daycares-Page__back-button">
                <img
                  className="Daycares-Page__arrow"
                  src={backArrow}
                  alt="back"
                />
              </button>
        ...
        {mode === "view" && (
          <>
            ...
            <input
              type="search"
              name="search"
              className="Daycares-Page__search-field"
              placeholder="Search..."
            />
          </>
        )}
